...
# plt.show()
# possible_peaks = list()
# possible_peakstime = list()

# avg = sum(cleanSig) / len(cleanSig)
# diff = (max(cleanSig) - min(cleanSig)) / 2
# r_peak_threshold = avg + diff
# maxima = scipy.signal.argrelextrema(cleanSig, np.greater)
...
# minima = scipy.signal.argrelmin(cleanSig)
...
# FILTER OUT THE LOWER VALUES
# CALCULATE PROMINENCE?
# GET BASELINE STAT i GUESS
# COMPARE TO PREVIOUS PEAK, IF IT IS SMALLER THAN 200MS, DELETE THE PREVIOUS ONE

# CALUCLATE PROMINENCE?

# possible_peaks = scipy.signal.find_peaks(cleanSig, height=avg, distance=50)
# possible_peaks = np.asarray(possible_peaks)
# possible_peaks = possible_peaks.tolist()
# possible_peaks = possible_peaks[0]

# take out the list of time values and create that later. find the average of all (peak or not?) values and use that
# as a threshold. delete all peaks below that threshhold and end up with only above peaks
# ORR sense for da dips in the ecg data. a certain gap between two peaks, keep the first peak.
# create a separate for loop for the possible peaks time list, compare the time value for first peak with time value
# from 2nd peak, if the difference is a certain amount (40 time thing), add the first time value to another list
# Probably don't need the possible peaks data set with the voltage reading
# SOLUTION for the 3rd peak, identical voltage reading in a row, not detected as a peak. accept this drawback lmao
# ahhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh


# L = len(possible_peakstime)
# y = 0
# probable_peaks = list()
# for t in possible_peakstime[:L-1]:
   # t2 = possible_peakstime[y+1]

  #  if cleanSig[t] > r_peak_threshold:  # (cleanSig[t]-cleanSig[t2]) > 0.6:  # ((t2 - t) >= 15) and
 #       probable_peaks.append(t)

#    y = y + 1
# print(possible_peakstime)
